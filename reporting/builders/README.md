...
Builders die uit `Project`, `Stage` en `AppState` herbruikbare rapport-fragmenten samenstellen: `ReportSection`-objecten met `TextBlock`s, `ReportField`s en `ReportTable`s, plus een HTML-preview. De builders zijn pure datatransformaties zonder Qt-afhankelijkheden en zonder state-mutatie.
...
| `html_preview_builder.py` | Rendert een `ReportPackage` naar een gestylede HTML-string voor de preview-tab. |

## Patroon

- **Input**: een `Project`, optioneel een `Stage`, of een `ReportPackage` plus instellingen uit `AppState`.
- **Output**: `ReportSection`-objecten (met `TextBlock`/`ReportField`/`ReportTable`/`ReportImageRequest`) of een HTML-string.
...
4. Roep de builder aan vanuit `ReportController` of een hoger niveau (zoals `DamwandHoofdstukBuilder`) en voeg het resultaat toe aan de `ReportPackage`.
5. Mocht een Word-/Excel-export nodig zijn, registreer dan de blok-`id`s in de relevante exporter onder `exporters/`.
